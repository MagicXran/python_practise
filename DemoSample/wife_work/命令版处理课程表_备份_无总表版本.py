...
'''
注意:
0. 首先将输入的excel全部单元格设置为 文本单元格;
1. 输入表的第一个sheet 必须为 课程表(也叫综合表);
2. 第二个sheet必须为补课表;
3. 且两个sheet中不含班级课程表
4. 生成的excel表位于C:\Project\下
'''
...
WEEK_LESSION_BEGIN = "A3"  # 数据其实单元格，一般是数据区域中最左上角的那个单元格。即第一个姓名所在的单元格。
WEEK_LESSION_END = "AP47"  # 数据最后一个单元格，一般是数据区域中最右下角的那个单元格。

'''补课表参数设置'''
ASC_LESSION_BEGIN = "A3"
'''补课表数据起始'''
ASC_LESSION_END = "AX32"
'''补课表数据终止'''
ASC_LESSION_SCOPE = ASC_LESSION_BEGIN + ":" + ASC_LESSION_END

ASC_MAX_CLASSES_NUM = 6
'''补课表最大班级数'''
ASC_MAX_DAY_NUM = 6  # A1~A6
'''补课表最大天数'''
...
class Syllabus(object):
    ...
    def sel_teacher_table(self):
        ...
        print("打印教师[{}]:{}".format(len(self.teacher_names), self.teacher_names))
        ...
        for tea_index in range(len(self.teacher_names)):
            ...
            while count < MAX_DAY_NUM:
                ...
                if row_datas_vec[tea_index][start + count * cycle:end + count * cycle] == 'None':
                    ...
                    for class_no_ in every_day:
                        if (int(class_no_.strip('班')) if any(char.isdigit() for char in class_no_) else -1) > 0:
                            pass
                count = count + 1
                print(
        ...
        print("所有老师一周课程 len:{},{}".format(len(teas_course), tea_course))
        print(tea_course)
        return
        """创建教师课程表"""

        export_path2 = r"C:\\Users\\ZDxia\\Desktop\\" + self.grade + "教师课程表.xlsx"
    ...
    def sel_classes_table(self):
        ...
        return
        """创建班级课程表的excel"""
        export_path = r"C:\\Users\\ZDxia\\Desktop\\" + self.grade + "班级课程表.xlsx"
        print('导出目录：' + export_path)
        ...
        for sht in sheet_arr:
            ...
            # sht.autofit() # 自动适应行高
            sht.range("A1:I7").api.RowHeight = 100
    ...
    def teacher_asc(self):
        ...
        tea_number = int(ASC_LESSION_END[2:]) if len(ASC_LESSION_END) > 3 else int(ASC_LESSION_END[1:])
        '''计算教师人数，[A2:AP32] vs. [A2:Z4] 考虑右侧单元格列号位数'''
        print("教师列最大行号：{0}".format(tea_number))

        # 遍历每行,即每位教师 row3 ~ row37 最多支持35位老师
        for i in range(int(ASC_LESSION_BEGIN[1:]), tea_number + 1):
            row_datas = list()
            # 所有列
        ...
        export_path3 = r"C:\\Users\\ZDxia\\Desktop\\" + self.grade + "教师补课表.xlsx"
            ...
            for cn_index in range(cyc_tea, len(teacher_names_vec)):
                ...
                if (count & 1) == 0:
                    ...
                    sheet_arr3[sht_index].range(
                        'L' + str(((rigth_start_row - 1) * 9 + 1))).value = '教师:{}补课表'.format(
    ...
    def classes_asc(self):
        ...
        self.sheet: xw.Sheet = self.wb.sheets[1]
        sheet_new = self.sheet.copy()
        # 指定复制的单元格范围
        source_range = self.sheet.range('A1')
        destination_range = sheet_new.range('B3')

        # 将单元格范围从 sheet1 复制到 sheet2
        ...
        for i in range(0, self.col_nums):
            ...
            class_max_row_number = int(ASC_LESSION_END[2:]) if len(ASC_LESSION_END) > 3 else int(ASC_LESSION_END[1:])
            '''计算教师人数，[A2:AP32] vs. [A2:Z4] 考虑右侧单元格列号位数'''
            # print("教师列最大行号：{0}".format(class_max_row_number))

            for j in range(int(ASC_LESSION_BEGIN[1:]), class_max_row_number + 1):
        ...
        """.:cvar......................................创建表........................................."""
        export_path4 = r"C:\\Users\\ZDxia\\Desktop\\" + self.grade + "班级补课表.xlsx"
        ...
        for sht in sheet_arr4:
            ...
            # sht.autofit() # 自动适应行高
            sht.range("A1:I8").api.RowHeight = 100
